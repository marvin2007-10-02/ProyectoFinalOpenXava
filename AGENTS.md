...
- Create the action class in the `ni.edu.uam.bfavocabulario.actions` package, typically extending `ViewBaseAction`
...
<controller name="YourFirstEntity">
    ...
    <action name="showAmount" mode="detail"
        class="ni.edu.uam.bfavocabulario.actions.ShowAmountAction"/>
</controller>
```

```java
package ni.edu.uam.bfavocabulario.actions;
...
- ✅ Place action classes under `ni.edu.uam.bfavocabulario.actions`
...
### Basic Test Example
```java
package ni.edu.uam.bfavocabulario.tests;
...
- Create a `dashboards` package (e.g., `ni.edu.uam.bfavocabulario.dashboards`) and place the `Dashboard` class and its auxiliary classes in it.
